...
namespace SqlFunctions
{
    ...
    public class CCond
    {
        ...
        public float Il_loss_win
        {
            set;
            get;
        }
        public float Ripple_loss_win
        {
            set;
            get;
        }
        public float Xtalk_loss_win
        {
            set;
            get;
        }
        public float Align_il
        {
            set;
            get;
        }
                ...
                if (dt.Rows != null)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        CCond cond = new CCond()
                        {
                            ...
                            Il_loss_win = (float)Convert.ToDouble(dr["il_loss_win"]),
                            Xtalk_loss_win = (float)Convert.ToDouble(dr["xtalk_loss_win"]),
                            Align_il = (float)Convert.ToDouble(dr["align_il"]),
                            Ripple_loss_win = (float)Convert.ToDouble(dr["ripple_loss_win"])
                        };
