...
namespace SarchPMS.Business.Draw
{
    public class DrawingCurve
    {
        ...
        public Bitmap DrawImage(double[,] RawData, double[] XData, string strTitle, string xAxisTest, string yAxisTest, bool bIL, float width, float high)
        {
            int intCurvesCount = RawData.GetLength(0);
            Curve2D cuv2D = new Curve2D();
            cuv2D.Values = RawData;
            cuv2D.XValues = XData;
            cuv2D.Width = width;
            cuv2D.Height = high;
            cuv2D.Channel_Number = intCurvesCount;

            string[] xLabel = new string[11];
            for (int i = 0; i < 11; i ++)
            {
                double span = (XData[XData.Length -2] - XData[0]) / 10;
                xLabel[i] = Math.Round(XData[0] + i * span, 2).ToString();
                //if (intCurvesCount == 10)
                //    xLabel[i] = Math.Round(XData[0] + i * 2, 2).ToString();
                //else if (intCurvesCount == 4)
                //    xLabel[i] = Math.Round(XData[0] + i * 9, 2).ToString();
            }
            if (bIL)
            {
                if (intCurvesCount == 10)
                {
                    cuv2D.YSliceBegin = -60;
                    cuv2D.YSliceValue = 12;
                }
                else if (intCurvesCount == 4)
                {
                    cuv2D.YSliceBegin = -20;
                    cuv2D.YSliceValue = 12;
                }
            }
            else
            ...
            cuv2D.Keys = xLabel;
            cuv2D.Title = strTitle;
            cuv2D.XAxisText = xAxisTest;
            cuv2D.YAxisText = yAxisTest;
            //cuv2D.CurveColors= new Color[] { Color.Red, Color.Green, Color.Yellow, Color.HotPink, Color.Red, Color.Green, Color.Yellow, Color.HotPink, Color.Red, Color.Green };
            cuv2D.CurveColors = new Color[] { Color.Red, Color.Green, Color.Blue, Color.HotPink, Color.Yellow, Color.Brown, Color.Yellow, Color.HotPink, Color.Red, Color.Green };
            cuv2D.Fit();
    ...
    public class Curve2D
    {
        ...
        private double[,] fltsValues;//= new double[] { 20.0f, 30.0f, 50.0f, 55.4f, 21.6f, 12.8f, 99.5f, 36.4f, 78.2f, 56.4f, 45.8f, 66.5f, 99.5f, 36.4f, 78.2f, 56.4f, 45.8f, 66.5f, 20.0f, 30.0f, 50.0f, 55.4f, 21.6f, 12.8f }; //值
        ...
        public void Fit()
        {
            ...
            {
                if (Values[0,i] < fltMinValue)
                {
                    fltMinValue = float.Parse(Values[0, i].ToString());
                }
                else if (Values[0,i] > fltMaxValue)
                {
                    fltMaxValue =float.Parse(Values[0, i].ToString());
                }
            }
        ...
        public Bitmap CreateImage()
        {
            ...
            int intCurvesCount = Values.GetLength(0);
            int intValuesCount = Values.GetLength(1);
            float[] fltXValues = new float[intValuesCount];

            //if (intValuesCount % intKeysCount == 0)
            //{
            for (int j = 0; j < intValuesCount; j++)
            {
                fltXValues[j] = float.Parse(XValues[j].ToString());
            }
            for (int i = 0; i < intCurvesCount; i++)
            {
                ...
                for (int j = 0; j < intValuesCount; j++)
                {
                    fltCurrentValues[j] =-float.Parse ( Values[i,j].ToString ());
                }
                try
                {
                    DrawContent(ref objGraphics, fltXValues, fltCurrentValues, clrsCurveColors[i]);
                }
                catch
                {
                }
        ...
        {
            float fltX = Width / 2-20;// - XSpace + XSlice / 2 - (XAxisText.Length - 1) * intFontSpace;
            float fltY = Height - YSpace/2;
            objGraphics.DrawString(XAxisText, new Font("宋体", FontSize), new SolidBrush(AxisTextColor), fltX, fltY);

            fltX = XSpace/2-10;
            fltY = Height/3;
            for (int i = YAxisText.Length-1; i >=0; i--)
            {
                objGraphics.TranslateTransform(fltX, fltY);
                objGraphics.RotateTransform(270, MatrixOrder.Prepend);

                objGraphics.DrawString(YAxisText[i].ToString(), new Font("宋体", FontSize), new SolidBrush(AxisTextColor), 0, 0);
        ...
        /// </summary>
        /// <param name="objGraphics"></param>
        private void DrawContent(ref Graphics objGraphics, float[] XValue,float [] YValue, Color clrCurrentColor)
        {
            Pen CurvePen = new Pen(clrCurrentColor, CurveSize);
            PointF[] CurvePointF = new PointF[XValue.Length];
            float keys = 0;
            float values = 0;
            float  span = (XValue[XValue.Length - 2] - XValue[0]) / 10;

            //float[] X = new float[XValues.Length];
            //float[] Y = new float[XValues.Length];
            for (int i = 0; i < XValue.Length; i++)
            {
                //if (Channel_Number == 4)
                //    keys = XSlice * (XValue[i] - XValue[0]) / 9 + XSpace;
                //else if (Channel_Number == 10)
                    keys = XSlice * (XValue[i] - XValue[0]) / span + XSpace;
                //values = (Height - YSpace) + YSliceBegin - YSlice * (fltCurrentValues[i] / YSliceValue);
                values = YSpace - YSlice * (YValue[i] / (YSliceValue));
                // values =Height -( YSpace  + YSlice * ((-YValue[i]-YSliceBegin) /( YSliceValue)));
                //X[i] = keys;
                //Y[i] = values;
        ...
        private void CreateTitle(ref Graphics objGraphics)
        {
            ...
            objGraphics.DrawString(Title, new Font("宋体", FontSize), new SolidBrush(TextColor), new Point((int)(Width/2), (int)(YSpace - YSlice / 2 )));
        }
    }
}
